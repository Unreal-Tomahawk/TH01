...
bool ATamMath::isPointInside2Points(FVector2D point1, FVector2D point2, FVector2D destPoint) {
	...
	if (dx > dy) {
		if (destPoint.X > __max(point1.X, point2.X)) return false;
		if (destPoint.X < __min(point1.X, point2.X)) return false;
	} else {
		if (destPoint.Y > __max(point1.Y, point2.Y)) return false;
		if (destPoint.Y < __min(point1.Y, point2.Y)) return false;
	}
	return true;
}
